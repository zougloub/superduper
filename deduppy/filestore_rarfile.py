...
class RarInfoStat(object):
	def __init__(self):
		self.st_size = 0

class FileStore_RarFile(filestore.FileStore):
	"""
	Files store for rarfile
	"""
	def __init__(self, root, max_files=None):
		self.root = root
		self._files = {} # name -> File
		self._open_files = {} # relpath -> file object
		self._handle = rarfile.RarFile(self.root, "r")

	def url(self):
		return 'rar://' + self.root + '/'
	...
	def __str__(self):
		return '(store path="rar://%s")' % (self.root)
